...
{
    public partial class PanelMember_signup : Form
    {
        public PanelMember_signup()
        {
            InitializeComponent();
        }
        ...
        private void button1_Click(object sender, EventArgs e)
        {
                ...
                using (SqlConnection connection = new SqlConnection("Data Source=DESKTOP-3TMHGIH\\MSSQLSERVER01;Initial Catalog=project;Integrated Security=True"))
                {
                    try
                    {
                        ...
                        string checkDuplicateQuery = "SELECT COUNT(*) FROM PanelMember WHERE Name = @Name";
                        ...
                        string checkPasswordQuery = "SELECT COUNT(*) FROM PanelMember WHERE Password = @Password";
                        ...
                        string insertQuery = "INSERT INTO PanelMember (Name, Password, Email, ContactNumber) VALUES (@Name, @Password, @Email, @ContactNumber)";
                        using (SqlCommand insertCommand = new SqlCommand(insertQuery, connection))
                        {
                            ...
                            if (rowsAffected > 0)
                            {
                                ...
                                MessageBox.Show("Panel member registration successful!", "Success", MessageBoxButtons.OK, MessageBoxIcon.Information);
                                // Add further logic here if needed
                            }
                            else
                            {
                                // Registration failed
                                MessageBox.Show("Panel member registration failed. Please try again.", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                            }
                        }
                    }
                    catch (Exception ex)
